...
const FormModelConverter = () => {
  ...
  const annotateSchema = (schema, layout) => {
    ...
    layout.getQuads(null, TERM_RdfType, TERM_LayoutType).forEach(quad => {
      const annotated = layout
        .getQuads(quad.subject, TERM_LayoutAnnotation, null)
        .map(t => {
          let elt = null;
          ...
          if (quads.length) {
            ...
            const pathStr = layout.getQuads(t.object, TERM_LayoutPath, null)[0]
              .object.value;
  ...
  const convertShacl = async () => {
    ...
    const response = await fetch(
      "https://jmartin.inrupt.net/public/shapes/book-shacl.ttl"
    );
    const shape = await response.text();
    const newResponse = await fetch(
      "https://jmartin.inrupt.net/public/books/book.ttl"
    );
      ...
      (e, report) => {
        if (report.conforms() === false) {
          ...
          report.results().forEach(result => {
            ...
            result.resultNode["http://www.w3.org/ns/shacl#resultPath"].forEach(
              m => {
                message += `${m["@id"]} `;
              }
            );
