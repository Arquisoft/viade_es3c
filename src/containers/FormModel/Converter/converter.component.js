...
const FormModelConverter = () => {
	...
	const filteredOptions = ConverterTypesList.filter((item) => t(`formLanguage.${item}`) === t("formLanguage.shex"));
   ...
   */
	const isShEx = (value) =>
		value === t(`formLanguage.${ConverterTypes.Shex}`) || value === t(`formLanguage.${ConverterTypes.ShexLayout}`);
			...
			<FormWrapper>
				...
				<Result>
					<ResultHeader>
						...
						<button type="button" onClick={copyToClipboard} data-testid="copy-button" disabled={!formModel}>
