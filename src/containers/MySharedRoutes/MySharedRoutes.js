...
const routePath = process.env.REACT_APP_VIADE_ES3C_SHARED_PATH;
...
class MyRoute extends React.Component {
	...
	componentDidMount() {
		...
		this._asyncRequest = viadeManager.readRoutesSharedFromPod(webId).then((data) => {
					...
					<MyRouteContainer data-testid="myroute-container">
						<FormRenderContainer>
							<Header data-testid="myroute-header">
								<h1>{i18n.t("navBar.mySharedRoutes")}</h1>
								<input
									type="text"
									className="input"
